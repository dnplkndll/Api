...
describe("ConversationRepo", () => {
  ...
  it("should create a conversation", async () => {
    ...
    convId = conv.id;
    // dateCreated and postCount are DB-generated (NOW(), 0) — not on the returned model
    const reloaded = await repo.loadById(churchId, convId);
    expect(reloaded.dateCreated).toBeInstanceOf(Date);
    expect(reloaded.postCount).toBe(0);
  });
