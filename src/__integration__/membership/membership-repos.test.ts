...
describe("PersonRepo", () => {
  ...
  it("should load a person by id", async () => {
    ...
    // load() returns raw DB row with flat columns; use convertToModel for nested
...
describe("AccessLogRepo", () => {
  ...
  it("should create an access log", async () => {
    const log = await repo.create({ churchId, userId: "testuser001", appName: "test" } as any);
...
describe("QuestionRepo", () => {
  ...
  it("should update a question", async () => {
    ...
    // choices is stored as JSON string in DB; parse for comparison
