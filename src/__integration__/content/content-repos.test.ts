...
describe("FileRepo", () => {
  ...
  it("should create a file", async () => {
    ...
    // dateModified is DB-generated via NOW() — not on the returned model
    fileId = file.id;
  });
  ...
  it("should loadTotalBytes", async () => {
    const row = await repo.loadTotalBytes(churchId, "website", "site1");
    expect(row).toBeDefined();
    expect(Number(row.size)).toBeGreaterThanOrEqual(0);
  });
