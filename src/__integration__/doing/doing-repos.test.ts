...
describe("TaskRepo", () => {
  ...
  it("should create with auto-increment taskNumber", async () => {
    ...
    expect(result.id).toBeDefined();
    taskId = result.id;
    // taskNumber is DB-generated — reload to verify
    const reloaded = await repo.load(churchId, taskId);
    expect(reloaded.taskNumber).toBeDefined();
  });
  ...
  it("should auto-increment taskNumber", async () => {
    ...
    const second = await repo.load(churchId, result.id);
