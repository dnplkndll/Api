...
export class ChurchController extends MembershipBaseController {
  ...
  public async select(req: express.Request<{}, {}, { churchId: string; subDomain: string }>, res: express.Response): Promise<any> {
    return this.actionWrapper(req, res, async (au) => {
      ...
      if (req.body.subDomain && !churchId) {
        const selectedChurch = await this.repos.church.loadBySubDomain(req.body.subDomain) as any as Church;
  ...
  private async fetchChurchPermissions(au: AuthenticatedUser, churchId: string): Promise<LoginUserChurch> {
    ...
    if (result === null) {
      const church = await this.repos.church.loadById(churchId) as any as Church;
      result = {
