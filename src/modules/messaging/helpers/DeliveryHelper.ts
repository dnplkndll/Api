...
export class DeliveryHelper {
  ...
  static sendConversationMessages = async (payload: PayloadInterface) => {
    const connections = DeliveryHelper.repos.connection.convertAllToModel(payload.churchId, await DeliveryHelper.repos.connection.loadForConversation(payload.churchId, payload.conversationId));
